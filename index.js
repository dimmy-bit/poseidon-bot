...
import { bannerMIR } from './bannerMIR.js';
dotenv.config();

// ====== Logger (keeps output friendly & colored) ======
const logger = {
  info: (msg, options = {}) => {
    const timestamp = new Date().toISOString().slice(0, 19).replace('T', ' ');
    const emoji = options.emoji || 'ℹ️  ';
    const context = options.context ? `[${options.context}] ` : '';
    const level = chalk.bgBlue.white(' INFO ');
    const formattedMsg = `${emoji} ${level} ${chalk.white(context)} ${chalk.white(msg)}`;
    console.log(formattedMsg);
  },
  warn: (msg, options = {}) => {
    const emoji = options.emoji || '⚠️  ';
    const level = chalk.bgYellow.black(' WARN ');
    console.log(`${emoji} ${level} ${chalk.yellow(msg)}`);
  },
  error: (msg, options = {}) => {
    const emoji = options.emoji || '❌  ';
    const level = chalk.bgRed.white(' ERROR ');
    console.log(`${emoji} ${level} ${chalk.red(msg)}`);
  }
};
...
async function countdownDelay() {
  ...
  const interval = setInterval(() => {
    ...
    else { clearInterval(interval); process.stdout.write('\r' + ' '.repeat(process.stdout.columns || 80) + '\r'); console.log(); }
  }, 1000);
...
async function formatCampaignTable(campaigns, context) {
  ...
  const rows = campaigns.map(campaign => {
    ...
    const tags = (campaign.tags && campaign.tags.join(', ') || 'N/A').slice(0, 7);
...
async function requestWithRetry(method, url, payload = null, config = {}, retries = 5, backoff = 5000, context) {
  for (let i = 0; i < retries; i++) {
    try {
      let response;
      if (method.toLowerCase() === 'get') response = await axios.get(url, config);
      else if (method.toLowerCase() === 'post') response = await axios.post(url, payload, config);
      else if (method.toLowerCase() === 'put') response = await axios.put(url, payload, config);
      else throw new Error(`Method ${method} not supported`);
      return { success: true, response: response.data };
    } catch (error) {
      const status = error.response?.status;
      if (status === 429) backoff = 30000;
      if (status === 400 || status === 404) return { success: false, message: error.response?.data?.message || 'Bad request', status };
      if (i < retries - 1) {
...
// Tokens read: prefer .env TOKENS, else fallback to token.txt
async function readTokens() {
  try {
    if (process.env.TOKENS && process.env.TOKENS.trim().length > 0) {
      const tokens = process.env.TOKENS.split(',').map(t => t.trim()).filter(Boolean);
      logger.info(`Loaded ${tokens.length} token${tokens.length === 1 ? '' : 's'} from .env`, { emoji: '📄 ' });
      return tokens;
    }
    const data = await fs.readFile('token.txt', 'utf-8');
    const tokens = data.split('\n').map(line => line.trim()).filter(line => line.length > 0);
    logger.info(`Loaded ${tokens.length} token${tokens.length === 1 ? '' : 's'} from token.txt`, { emoji: '📄 ' });
    return tokens;
  } catch (error) {
    logger.error(`Failed to read tokens: ${error.message}`, { emoji: '❌ ' });
    return [];
  }
}
...
async function readProxies() {
  try {
    ...
    if (proxies.length === 0) logger.warn('No proxies found. Proceeding without proxy.', { emoji: '⚠️  ' });
    else logger.info(`Loaded ${proxies.length} prox${proxies.length === 1 ? 'y' : 'ies'}`, { emoji: '🌐  ' });
    return proxies;
  } catch (error) {
...
async function generateAudioBuffer(text, lang) {
  ...
  console.log = () => {}; // suppress node-gtts logs
  if (lang === 'mr' || lang === 'ur') lang = 'hi';
...
async function processToken(token, index, total, proxy = null) {
  ...
  for (let campIndex = 0; campIndex < campaigns.length; campIndex++) {
    ...
    const campaignContext = `${context}|${(campaign.campaign_name||'campaign').slice(0, 10)}`;
    ...
    if (quota.remaining > 0) {
      ...
      while (currentRemaining > 0 && i < quota.cap) {
        ...
        i++;
        if (currentRemaining > 0) await delay(15); // small delay between uploads
      }
  ...
  if (uploads.length > 0) await formatUploadTable(uploads, context);
  else logger.info(chalk.bold.yellowBright('No uploads performed.'), { context });
...
// ====== helper askQuestion (for fallback) ======
function askQuestionCLI(query) {
...
async function mainMenuFlow() {
  ...
  if (!tokens || tokens.length === 0) {
    logger.error('No tokens found. Add tokens to .env (TOKENS) or token.txt and re-run.');
    process.exit(1);
  }
  ...
  const mainChoices = [
    { name: '🚀 Start ALL accounts (sequential)', value: 'start_all' },
    { name: '🧩 Start Single Account', value: 'start_single' },
    { name: '📊 Check Account Info', value: 'info' },
  ...
  while (keepRunning) {
    ...
    if (ans.action === 'start_all') {
      logger.info(`Starting all ${tokens.length} accounts (sequential)`);
      for (let i = 0; i < tokens.length; i++) {
        const proxy = proxies.length ? proxies[i % proxies.length] : null;
        try { await processToken(tokens[i], i, tokens.length, proxy); }
        catch (e) { logger.error(`Error processing account ${i+1}: ${e.message}`); }
      }
    } else if (ans.action === 'start_single') {
      const pick = await inquirer.prompt([{
        type: 'list',
        name: 'which',
        message: 'Choose account (by index):',
        choices: tokens.map((t, idx) => ({ name: `Account ${idx+1}`, value: idx }))
      }]);
      const idx = pick.which;
      const proxy = proxies.length ? proxies[idx % proxies.length] : null;
      await processToken(tokens[idx], idx, tokens.length, proxy);
    } else if (ans.action === 'info') {
      const pick = await inquirer.prompt([{
        type: 'list',
        name: 'which',
        message: 'Choose account for info:',
        choices: tokens.map((t, idx) => ({ name: `Account ${idx+1}`, value: idx }))
      }]);
      const idx = pick.which;
      const proxy = proxies.length ? proxies[idx % proxies.length] : null;
      const info = await fetchUserInfo(tokens[idx], proxy, `Account ${idx+1}/${tokens.length}`);
      console.log('\n');
      printHeader(`User Info`);
      printInfo('Username', info.username);
      printInfo('Address', info.address);
      printInfo('Points', info.points);
    } else if (ans.action === 'exit') {
